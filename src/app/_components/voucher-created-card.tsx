      ...
      >
        <div className="flex flex-col gap-6">
          <p className="font-medium text-primary-100">
            Ocorreu um erro ao criar o voucher, por favor, volte e tente
            novamente
          </p>
          <Button
            onClick={() => handleClick(true)}
            className="mb-4 h-12 text-primary-50"
          >
    ...
    try {
      ...
      if (!voucher) {
        toast({
          ...
          description:
            "Não foi possível encontrar o voucher. Recarregando a página...",
          variant: "destructive",
        });
      ...
      if (payment_success_url) {
        ...
        toast({
          title: "Erro",
          description:
            "URL de sucesso não disponível. Recarregando a página...",
          variant: "destructive",
        });
        setTimeout(() => location.reload(), 2000);
      }
      ...
      handleError(
        error,
        "Erro ao verificar o status do pagamento. Tente novamente.",
      );
    } finally {
  ...
  function AlreadyPayedButton() {
    return (
      <div className="flex flex-col gap-4 text-center text-lg">
        ...
        <Button
          className="h-16 w-full rounded-xl text-xl"
        ...
        {payment_success_url ? (
          <div className="p-4">
            <p className="mb-4 text-xl font-medium text-green-100">
              Já recebemos seu pagamento, clique no botão abaixo para visualizar
              o voucher:
            </p>
            <Button
              className="h-14 w-full bg-positive-green text-xl"
              onClick={() => window.open(payment_success_url)}
            >
              Visualizar voucher
            </Button>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="w-12">
              <DeleteVoucherCookieBtn
                label="Voltar"
                refresh={true}
                message="Se você já pagou e ainda não está vendo o botão para visualizar o voucher, atualize a pagina e tente novamente."
              />
            </div>
            <p className="font-medium text-primary-100">
              Voucher criado com sucesso! Guarde o codigo abaixo, finalize o
              pagamento clicando no botão abaixo e volte ao site para
              utiliza-lo:
            </p>
            <h2 className="text-center text-7xl font-bold text-primary-50">
              {code}
            </h2>
            <Button
              onClick={checkPaymentStatus}
              disabled={isCheckingPayment || !isOnline}
              className="flex h-14 w-full items-center justify-center rounded-xl bg-positive-green text-center text-xl font-medium text-primary-50 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isCheckingPayment ? (
