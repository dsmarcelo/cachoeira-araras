...
describe("payment refunds", () => {
  ...
  test("a partial provider refund is never recorded as completed", async () => {
    const t = createConvexTest();
    fake = createMercadoPagoFake();
    fake.payments.set("pay-partial", {
      id: "pay-partial",
      status: "approved",
      externalReference: "REF001",
      amount: 50,
    ...
    expect(refund?.status).toBe("needs_retry");
    expect(refund?.completedAt).toBeUndefined();
  });
  ...
  test("a sweep recovers overdue refunds that lost their scheduled attempt", async () => {
    ...
    const sweptCount = await t.mutation(
