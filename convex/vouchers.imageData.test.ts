...
function imageDataRequest(body: Record<string, unknown>, secret: string | undefined) {
...
test("getVoucherForImage has no public entry point a signed-in caller can reach", () => {
  ...
  // read name/phone through the client SDK no matter what code it holds.
...
test("the image-data door rejects a request with no shared secret", async () => {
  ...
  const response = await t.fetch(
    ...
    imageDataRequest({ code: "a1b2" }, undefined),
  );
...
test("the image-data door rejects a wrong shared secret", async () => {
  ...
  const response = await t.fetch(
    "/services/voucher-image-data",
    imageDataRequest({ code: "a1b2" }, "not-the-secret"),
  );

  expect(response.status).toBe(401);
});

test("the image-data door returns the full record for a real code with the correct secret", async () => {
  const t = convexTest(schema, modules);
  await insertVoucher(t);
...
test("an unknown code returns null rather than erroring", async () => {
  ...
  const response = await t.fetch(
    "/services/voucher-image-data",
    imageDataRequest({ code: "zzzz" }, testSecret),
  );
...
test("a soft-deleted voucher's code returns null", async () => {
  ...
  const response = await t.fetch(
    "/services/voucher-image-data",
    imageDataRequest({ code: "gone" }, testSecret),
  );
